...
loans = pd.read_csv('C:/Users/003ZCV744/Documents/Jupyter Notebooks/Course materials/Refactored_Py_DS_ML_Bootcamp-master/15-Decision-Trees-and-Random-Forests/loan_data.csv')
...
loans[loans['credit.policy']==1]['fico'].hist(alpha=0.5, bins=30, color='blue', label='CreditPolicy1')
loans[loans['credit.policy']==0]['fico'].hist(alpha=0.5, bins=30, color='red', label='CreditPolicy0')
plt.legend()
...
final_data = pd.DataFrame(pd.get_dummies(loans, columns=cat_feats, drop_first=True))
